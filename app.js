...
const TOOL_TABS = [
  ...
  { key: 'customs', ic: 'warn',     label: '通關須知' },
  { key: 'festival', ic: 'shrine',  label: '祭典行事曆' },
];
let toolTab = 'fx';
...
function showToolbox(tab = 'fx') {
  ...
  view.innerHTML = `
    <div class="pv-header">
      ...
      <div class="pv-name-en">匯率換算 · 打包清單 · 季節速查 · 天氣預報 · 預算估算 · 花費記帳 · 通關須知，出發前後一次備齊</div>
    </div>
    <div class="pv-content" id="pv-content">
      <div class="tool-tabs">
...
function renderToolTab() {
  ...
  else if (toolTab === 'customs') renderCustomsTool(body);
  else if (toolTab === 'festival') renderFestivalTool(body);
}
...
// ── 通關須知 ──────────────────────────────────────────────────────────────────
// 出發前／入境日本／離開日本前／回台灣四階段，依風險分三級：
// danger＝完全不能帶、warning＝有條件限制要注意、info＝額度／流程類提醒
// phase 只標在每個階段第一張卡上，render 時遇到 phase 變化才畫分隔線
const CUSTOMS_INFO = [
  { phase: '出發前', title: '出發前・Visit Japan Web 電子入境', level: 'info', items: [
    '出發前上網登入 Visit Japan Web，一次填好護照資料、航班資訊、住宿地址、入境目的、海關申報表（入境卡＋申告書電子化二合一）',
    '建議在抵達日本前<strong class="cst-key">最晚 6 小時</strong>完成，入境時直接出示 QR Code，走快速通關／電子閘門',
    '目前支援機場：羽田（第 2、3 航廈）、成田（第 3 航廈）、關西（第 1、2 航廈）、福岡機場',
  ]},
  { phase: '入境日本', title: '入境日本・絕對禁止攜帶', level: 'danger', items: [
    '生肉、真空包裝肉品、肉乾／香腸／培根／肉包（牛豬羊鹿等偶蹄類、雞鴨鵝等家禽全都不行，<strong class="cst-key">飛機上沒吃完的肉類餐點也算</strong>）',
    '生蛋、溫泉蛋、皮蛋（<strong class="cst-key">連蛋殼也不行</strong>）',
    '海鮮活體、未經處理的新鮮水果與植物種子',
  ]},
  { title: '入境日本・免稅範圍', level: 'info', items: [
    '酒類：合計 <strong class="cst-key">3 瓶以內</strong>（每瓶 760ml 以下）',
    '捲菸：<strong class="cst-key">200 支以內</strong>（其他菸品比例換算）',
    '其他物品：總值 <strong class="cst-key">20 萬日圓以內</strong>免稅，超過需主動申報',
    '乳製品（起司、保久乳）限非營業用途、<strong class="cst-key">10 公斤以下</strong>',
  ]},
  { title: '入境日本・藥品與現金申報', level: 'warning', items: [
    '成藥每種<strong class="cst-key">最多 12 瓶／盒／罐</strong>，合計不超過 36 瓶／盒／罐',
    '含偽麻黃鹼（比例超過 10%）、可待因等成分的感冒藥、止咳藥可能被視為管制藥物，建議只帶足夠天數的量，並保留藥品原包裝',
    '攜帶<strong class="cst-key">超過 100 萬日圓現金</strong>（或等值有價證券）需向海關申報',
  ]},
  { title: '手提行李安檢・液體限制', level: 'info', items: [
    '單一容器<strong class="cst-key">不超過 100ml</strong>，全部裝進一個 1 公升透明夾鏈袋（約 20cm×20cm 以內）才能帶上機',
    '超過 100ml 的液體（保養品、飲料、瓶裝水）<strong class="cst-key">不能手提</strong>，請放進託運行李',
    '機場免稅店買的液體會用防拆密封袋（STEBs）處理，若行程中有轉機要特別留意能否過關',
  ]},
  { title: '手提行李安檢・行動電源與鋰電池', level: 'warning', items: [
    '2026 年 4 月 24 日起日本新規：行動電源與備用鋰電池額定容量須在 <strong class="cst-key">160Wh 以下</strong>，超過全面禁止攜帶',
    '每人<strong class="cst-key">最多攜帶 2 顆</strong>（行動電源＋備用電池合計），且<strong class="cst-key">一律不能放託運行李</strong>，只能隨身帶上機',
    '登機後要放在座位口袋、隨身小包等看得到的地方，不可收進頭頂行李櫃；機上禁止幫行動電源充電，也不能用它幫其他裝置供電',
    '每顆都要做防短路處理：貼絕緣膠帶封住接口，或裝進原廠盒／夾鏈袋，避免跟鑰匙硬幣等金屬物混放',
    '換算公式：Wh＝mAh × 電壓(V) ÷ 1000，多數行動電源標示 3.7V，買之前先算一下別超過 160Wh',
  ]},
  { phase: '離開日本前', title: '離開日本前・免稅退稅注意事項', level: 'warning', items: [
    '現行制度（2026 年 11 月前）：消耗品（化妝品、藥品、食品等）會裝進特製免稅封套，出境前不能拆封，<strong class="cst-key">一旦拆封或已使用會被認定為已消費</strong>，需在海關補繳消費稅',
    '海關會核對系統購物紀錄，高單價商品有權要求現場出示；免稅品要隨身攜帶，不能請人代收或先寄回台灣（<strong class="cst-key">2025/4/1 起已禁止免稅品「另行寄送」</strong>）',
    '免稅品須在購買日起 <strong class="cst-key">90 天內</strong>攜帶出境，並在離境時經海關確認才符合退稅資格',
    '<strong class="cst-key">2026 年 11 月 1 日起制度將改為「先付後退」</strong>：消費時先付含稅全額，出境前持護照與商品向海關確認後才退還消費稅，消耗品屆時也不用再裝密封袋，出發前請留意當時最新流程',
  ]},
  { title: '離開日本前・刀具與尖銳物品', level: 'warning', items: [
    '菜刀、剪刀、美工刀、鑰匙圈小刀等有刃物品，<strong class="cst-key">一律不能放隨身行李</strong>，安檢直接沒收不會歸還',
    '在日本買刀具、廚房剪、指甲刀等，記得收進託運行李，用刀套或報紙包好避免刺破行李袋',
    '託運行李件數與重量限制依航空公司而異，大量採購刀具前先查清楚，避免超重加收費用',
  ]},
  { phase: '回台灣', title: '回台灣・絕對禁止帶回', level: 'danger', items: [
    '所有豬肉、雞肉相關的香腸、火腿腸、臘腸、肉乾、肉鬆等肉製品零食，不論加工與否一律禁止（防堵非洲豬瘟，<strong class="cst-key">違規最重罰新台幣 100 萬元</strong>）',
    '電子煙、加熱菸（日本合法販售，但<strong class="cst-key">一律不能帶回台灣</strong>）',
  ]},
  { title: '回台灣・免稅額度與現金申報', level: 'info', items: [
    '自用行李物品免稅額：每人 <strong class="cst-key">NT$35,000</strong>',
    '新台幣現金上限 <strong class="cst-key">10 萬元</strong>，超過部分不論是否申報一律不准攜帶',
    '外幣現金等值超過 <strong class="cst-key">1 萬美元</strong>（或人民幣超過 2 萬元）需申報',
  ]},
  { title: '回台灣・貴重商品與奢侈品申報', level: 'warning', items: [
    '一般自用行李物品免稅額 NT$35,000 是所有購物加總計算，不是單件分開算',
    '已拆封使用過的單件物品（例如已背過的名牌包），完稅價格在 <strong class="cst-key">NT$10,000 以下</strong>可視為舊品免稅、不併入 35,000 額度，建議拆吊牌並實際使用過再入境',
    '超過額度須主動申報，依關稅、貨物稅、營業稅（約完稅價格加稅後再乘 5%）計算補稅，保留購買收據可加速核估',
    '常態性大量代購或有營利行為者不適用免稅額，可能被認定為進口貨物課稅',
  ]},
];

function renderCustomsTool(body) {
  let lastPhase = null;
  body.innerHTML = `
    <div class="tool-card">
      <div class="tool-card-head">出入境通關須知</div>
      ${CUSTOMS_INFO.map(g => {
        const phaseHtml = g.phase && g.phase !== lastPhase ? `<div class="cst-phase${lastPhase === null ? ' first' : ''}">${g.phase}</div>` : '';
        lastPhase = g.phase || lastPhase;
        return `${phaseHtml}
        <div class="cst-card" data-level="${g.level}">
          <div class="cst-head">
            <span class="cst-badge">${g.level === 'danger' ? '禁止' : g.level === 'warning' ? '注意' : '額度'}</span>
      ...
      <div class="cst-note">規定會隨政策調整，實際以出發前台灣關務署、日本稅關官方公告為準；行李有疑慮時主動向海關申報，永遠比被查到沒申報安全。</div>
    </div>`;
}

// ── 祭典行事曆 ──────────────────────────────────────────────────────────────
// 依季節分組，date/desc/fact 都經 WebSearch 查證（非憑印象寫），日期為每年慣例期間，
// 實際以官方公告為準（部分如三社祭、隅田川花火大會每年會落在該月同一週的週末，日期略有浮動）
const FESTIVALS = [
  { season: 'spring', name: '高山祭（山王祭・八幡祭）', jp: '高山祭 たかやままつり', pref: '岐阜県高山市', date: '4/14〜15（春）、10/9〜10（秋）',
    desc: '江戶時代傳承下來的絢爛屋台（山車）遶行古街，日本三大美祭之一。春天是日枝神社的山王祭、秋天是櫻山八幡宮的八幡祭，兩場加起來近 20 台屋台，部分屋台上還裝著會動的からくり人形。',
    fact: '屋台上的からくり人形是靠人力同時牽引十幾條絲線操演的木偶戲，操偶師要練好幾年才能上場，堪稱江戶時代的機械工程展示。2016 年「山・鉾・屋台行事」整組登錄 UNESCO 無形文化遺產，高山祭也在其中。' },
  { season: 'spring', name: '三社祭', jp: '三社祭 さんじゃまつり', pref: '東京都・淺草', date: '5 月第三個週末（2026 年 5/15〜17）',
    desc: '淺草神社例大祭，第二、三天上百座神轎輪番出動，重達 1 噸的本社神轎由 70 幾名轎夫扛著在雷門周邊街道狂野遊行，是東京最有江戶味的一場祭典。',
    fact: '扛神轎的轎夫身上刺青很常見，傳統上跟江戶「町火消」職人文化有關，三社祭也是少數會讓遊客近距離看到這層傳統文化的場合。' },
  { season: 'spring', name: '博多どんたく港まつり', jp: '博多どんたく', pref: '福岡県福岡市', date: '5/3〜4（黃金週）',
    desc: '起源自 850 年前的「博多松囃子」，是「參加型」而非「觀賞型」的祭典，企業、學校、社區團體都能報名遊行隊伍，整座城市那兩天像全民同樂會。',
    fact: '遊行隊伍裡常見有人手拿飯匙（しゃもじ）邊走邊敲，是どんたく的招牌道具，據說源自以前主婦們隨手拿廚房用具就上街同樂的傳統。' },
  { season: 'summer', name: '祇園祭', jp: '祇園祭 ぎおんまつり', pref: '京都府京都市', date: '7 月整月（前祭 7/14〜17、後祭 7/21〜24）',
    desc: '京都八坂神社的祭典，日本三大祭之一，前後歷時一個月，重頭戲是超過 30 台裝飾華麗、部分高達 25 公尺的山鉾在市區巡行，夜晚點燈的「宵山」街景是整場祭典最浪漫的時刻。',
    fact: '祇園祭已經辦了 1100 多年，起源是西元 869 年京都爆發瘟疫，人們用 66 支鉾（代表當時日本 66 國）遊行祈求消災，山鉾巡行的傳統就這樣傳了下來。' },
  { season: 'summer', name: '天神祭', jp: '天神祭 てんじんまつり', pref: '大阪府大阪市', date: '7/24〜25',
    desc: '大阪天滿宮例大祭，日本三大祭之一，號稱世界最大水上祭典：白天神轎遊行，入夜後上百艘船隊在大川上巡遊，河面同時施放約 5000 發煙火，燈光倒映水面是天神祭的招牌畫面。',
    fact: '船渡御的傳統可以追溯到平安時代，最初是為了讓神明搭船巡視信徒的聚落，演變到現在成了全大阪最盛大的水上派對。' },
  { season: 'summer', name: '隅田川花火大会', jp: '隅田川花火大会', pref: '東京都・隅田川', date: '7 月最後一個週六（2026 年 7/25）',
    desc: '東京歷史最悠久的花火大會，兩個會場合計施放約 2 萬發煙火，隅田川兩岸從傍晚就擠滿人潮，是東京夏天最具代表性的煙火盛典。',
    fact: '前身可以追溯到 1733 年江戶時代的「両國川開き」，當時是為了祭祀飢荒瘟疫的犧牲者、驅除惡靈才施放煙火，算是全日本歷史最悠久的花火大會之一。' },
  { season: 'summer', name: 'ねぶた祭', jp: '青森ねぶた祭', pref: '青森県青森市', date: '8/2〜7',
    desc: '東北三大祭之一，巨大的人形燈籠山車「ねぶた」在夜裡點燈遊行，隊伍前方成群的「跳人」邊跳邊喊「ラッセラー」，最後一晚得獎的ねぶた還會搭船在青森港夜航，同時施放上萬發煙火作結。',
    fact: 'ねぶた的由來眾說紛紜，一種說法是源自「眠り流し」——把夏天讓人昏昏欲睡的睡意做成燈籠放水流走，才有精神迎接秋收農忙。' },
  { season: 'summer', name: '阿波おどり', jp: '阿波おどり', pref: '徳島県徳島市', date: '8/12〜15（前夜祭 8/11）',
    desc: '日本規模最大的舞蹈祭典，有 400 年歷史，上千個叫做「連」的舞蹈團體、約 10 萬名舞者走上街頭，男舞矯健、女舞優雅，一整座城市跳到瘋狂。',
    fact: '阿波おどり有一句經典口號「跳的人是傻瓜，看的人也是傻瓜，同樣是傻瓜不跳就虧了」，不少會場其實歡迎遊客臨時加入「連」一起跳。' },
  { season: 'autumn', name: '長崎くんち', jp: '長崎くんち', pref: '長崎県長崎市', date: '10/7〜9',
    desc: '長崎諏訪神社例大祭，國家指定重要無形民俗文化財。長崎自江戶時代就是對外通商口岸，祭典裡揉合了中國、荷蘭、葡萄牙的異國元素，最著名的「龍踊」由舞者操控長龍狂舞搶珠，帶著濃濃南蠻風情。',
    fact: '長崎くんち每年只由幾個特定的「踊町」輪流負責演出主秀，一輪要輪 7 年才會再次輪到自己的町，同一個節目通常要等 7 年才看得到一次。' },
  { season: 'autumn', name: '時代祭', jp: '時代祭 じだいまつり', pref: '京都府京都市', date: '10/22',
    desc: '平安神宮的祭典，京都三大祭之一，特色是長達 2 公里、約 2000 人參與的「時代行列」，隊伍裡的服裝從明治維新一路排到延曆時代，等於一場活生生的日本服裝史遊行。',
    fact: '每套服裝、道具都由京都職人依史料考證原樣復原，被稱為「活的風俗博物館」；隊伍行進順序還刻意倒著排——從最近的明治時代走到最古老的延曆時代。' },
  { season: 'winter', name: 'さっぽろ雪まつり', jp: 'さっぽろ雪まつり', pref: '北海道札幌市', date: '2 月上旬（2026 年 2/4〜11）',
    desc: '北海道代表性的冬季盛典，大通公園沿線出現上百座雪雕與冰雕，好幾座直逼建築物高度的大雪像最為壯觀，每年吸引超過 200 萬人次造訪，是全世界規模數一數二的雪祭。',
    fact: '雪祭最初只是 1950 年當地高中生在大通公園堆了 6 座雪雕的地方活動，沒想到隔年就吸引 5 萬人來看，之後越辦越大變成現在的國際級祭典。' },
  { season: 'winter', name: 'なまはげ柴灯まつり', jp: 'なまはげ柴灯まつり', pref: '秋田県男鹿市', date: '2 月第二個週五〜日（2026 年 2/13〜15）',
    desc: '在真山神社舉行，把男鹿地方相傳 900 多年、除夕夜挨家挨戶警告小孩「有沒有不聽話的孩子」的鬼神「なまはげ」信仰，和神社的柴燈祭結合成觀光祭典，壓軸是 15 尊なまはげ排成一列從雪山緩緩走下山，場面魄力十足又帶點神秘。',
    fact: 'なまはげ其實不是妖怪也不是鬼，在傳說裡它是山神的使者，專門提醒大人小孩不要偷懶、要認真過生活，某種程度上更像「凶狠版的年終考核委員」。' },
];

function renderFestivalTool(body) {
  let lastSeason = null;
  body.innerHTML = `
    <div class="tool-card">
      <div class="tool-card-head">日本祭典行事曆</div>
      ${FESTIVALS.map(f => {
        const s = SEASONS.find(x => x.key === f.season);
        const phaseHtml = f.season !== lastSeason
          ? `<div class="fes-phase${lastSeason === null ? ' first' : ''}" style="--sc:${s.color}">${s.label}・${s.sub}</div>` : '';
        lastSeason = f.season;
        return `${phaseHtml}
        <div class="fes-card" style="--sc:${s.color}">
          <div class="fes-head">
            <span class="fes-date">${UI_ICONS.calendar}${f.date}</span>
            <span class="fes-pref">${f.pref}</span>
          </div>
          <div class="fes-title">${f.name}<span class="fes-name-jp">${f.jp}</span></div>
          <div class="fes-desc">${f.desc}</div>
          <div class="fes-fact">${UI_ICONS.bulb}<span>${f.fact}</span></div>
        </div>`;
      }).join('')}
      <div class="cst-note">日期為每年慣例期間，部分祭典（如三社祭、隅田川花火大会）每年落在該月同一週的週末，實際日期以官方公告為準。</div>
    </div>`;
}
