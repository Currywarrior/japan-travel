...
const CUSTOMS_INFO = [
  { phase: '出發前', title: '出發前・Visit Japan Web 電子入境', level: 'info', items: [
    ...
    '建議在抵達日本前最晚 6 小時完成，入境時直接出示 QR Code，走快速通關／電子閘門',
    '目前支援機場：羽田（第 2、3 航廈）、成田（第 3 航廈）、關西（第 1、2 航廈）、福岡機場',
  ]},
  { phase: '入境日本', title: '入境日本・絕對禁止攜帶', level: 'danger', items: [
    '生肉、真空包裝肉品、肉乾／香腸／培根／肉包（牛豬羊鹿等偶蹄類、雞鴨鵝等家禽全都不行，飛機上沒吃完的肉類餐點也算）',
    '生蛋、溫泉蛋、皮蛋（連蛋殼也不行）',
    '海鮮活體、未經處理的新鮮水果與植物種子',
  ]},
  { title: '入境日本・免稅範圍', level: 'info', items: [
    '酒類：合計 3 瓶以內（每瓶 760ml 以下）',
    '捲菸：200 支以內（其他菸品比例換算）',
    '其他物品：總值 20 萬日圓以內免稅，超過需主動申報',
    '乳製品（起司、保久乳）限非營業用途、10 公斤以下',
  ]},
  { title: '入境日本・藥品與現金申報', level: 'warning', items: [
    '成藥每種最多 12 瓶／盒／罐，合計不超過 36 瓶／盒／罐',
    '含偽麻黃鹼（比例超過 10%）、可待因等成分的感冒藥、止咳藥可能被視為管制藥物，建議只帶足夠天數的量，並保留藥品原包裝',
    '攜帶超過 100 萬日圓現金（或等值有價證券）需向海關申報',
  ]},
  { title: '手提行李安檢・液體限制', level: 'info', items: [
    '單一容器不超過 100ml，全部裝進一個 1 公升透明夾鏈袋（約 20cm×20cm 以內）才能帶上機',
    '超過 100ml 的液體（保養品、飲料、瓶裝水）不能手提，請放進託運行李',
    '機場免稅店買的液體會用防拆密封袋（STEBs）處理，若行程中有轉機要特別留意能否過關',
  ]},
  { title: '手提行李安檢・行動電源與鋰電池', level: 'warning', items: [
    '2026 年 4 月 24 日起日本新規：行動電源與備用鋰電池額定容量須在 160Wh 以下，超過全面禁止攜帶',
    '每人最多攜帶 2 顆（行動電源＋備用電池合計），且一律不能放託運行李，只能隨身帶上機',
  ...
  { phase: '離開日本前', title: '離開日本前・免稅退稅注意事項', level: 'warning', items: [
    '現行制度（2026 年 11 月前）：消耗品（化妝品、藥品、食品等）會裝進特製免稅封套，出境前不能拆封，一旦拆封或已使用會被認定為已消費，需在海關補繳消費稅',
    '海關會核對系統購物紀錄，高單價商品有權要求現場出示；免稅品要隨身攜帶，不能請人代收或先寄回台灣（2025/4/1 起已禁止免稅品「另行寄送」）',
    '免稅品須在購買日起 90 天內攜帶出境，並在離境時經海關確認才符合退稅資格',
    '2026 年 11 月 1 日起制度將改為「先付後退」：消費時先付含稅全額，出境前持護照與商品向海關確認後才退還消費稅，消耗品屆時也不用再裝密封袋，出發前請留意當時最新流程',
  ]},
  { title: '離開日本前・刀具與尖銳物品', level: 'warning', items: [
    '菜刀、剪刀、美工刀、鑰匙圈小刀等有刃物品，一律不能放隨身行李，安檢直接沒收不會歸還',
  ...
  { phase: '回台灣', title: '回台灣・絕對禁止帶回', level: 'danger', items: [
    '所有豬肉、雞肉相關的香腸、火腿腸、臘腸、肉乾、肉鬆等肉製品零食，不論加工與否一律禁止（防堵非洲豬瘟，違規最重罰新台幣 100 萬元）',
    '電子煙、加熱菸（日本合法販售，但一律不能帶回台灣）',
  ]},
  { title: '回台灣・免稅額度與現金申報', level: 'info', items: [
    '自用行李物品免稅額：每人 NT$35,000',
    '新台幣現金上限 10 萬元，超過部分不論是否申報一律不准攜帶',
    '外幣現金等值超過 1 萬美元（或人民幣超過 2 萬元）需申報',
  ]},
  { title: '回台灣・貴重商品與奢侈品申報', level: 'warning', items: [
    '一般自用行李物品免稅額 NT$35,000 是所有購物加總計算，不是單件分開算',
    '已拆封使用過的單件物品（例如已背過的名牌包），完稅價格在 NT$10,000 以下可視為舊品免稅、不併入 35,000 額度，建議拆吊牌並實際使用過再入境',
